...
class HumanDataset(Dataset):
    ...
    def read_images(self,index):
        ...
        try:
            ...
            if config.channels == 4:
                images = np.zeros(shape=(2048,2048,4))
            else:
                images = np.zeros(shape=(2048,2048,3))
            colors = ["red", "green", "blue"]
            if config.channels == 4:
                colors.append("yellow")
            images = [cv2.imread(filename+"_"+color+".jpg", cv2.IMREAD_GRAYSCALE).astype(np.uint8) for color in colors]
            images = np.stack(images, axis = -1)
            images = images.astype(np.uint8)
            # images = np.stack(images,-1) 
            if config.img_height == 2048:
                return images
            else:
                return cv2.resize(images,(config.img_weight,config.img_height))
